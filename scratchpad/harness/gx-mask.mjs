/* gx-mask.mjs — how deep is the mask really, measured WITHOUT decoding the
 * tone curve.
 *
 * Subtracting sRGB-decoded bytes is wrong here: the composite runs ACES plus a
 * contrast, a black point and a lift, so a byte is not proportional to scene
 * radiance and "linear" arithmetic on it silently rescales every ratio. So this
 * calibrates instead. With the fill removed, the sun's intensity is swept over
 * a ladder of known multipliers and the OPEN pad's byte is recorded at each —
 * that curve is the whole transfer from scene-linear key to pixel, whatever is
 * in it. The shadowed pad's byte is then inverted through that same curve, and
 * the multiplier it lands on IS the mask.
 *
 *   node gx-mask.mjs [--cam far] [--time 9]
 */
...
const pick = await page.evaluate(() => {
  ...
  for (let sy = 0; sy < innerHeight; sy += 2) for (let sx = 0; sx < innerWidth; sx += 2) {
    ...
    if (fs < 0.02 && hit) { sets.deep.push({sx, sy}); deepXZ.push(+fs.toFixed(3)); }
    else if (fs > 0.99 && !hit) sets.open.push({sx, sy});
  }
  return {sets, sunI: gi.sun.intensity, nDeep: sets.deep.length, nOpen: sets.open.length};
});
...
/* freeze the stop, remove the fill, keep the cascades from being redrawn */
...
/* the shadowed reading, with the mask live */
const live = await readPixels();

/* the calibration ladder, with the mask gated OFF so `k` is the only variable */
...
const KS = [0, 0.04, 0.08, 0.12, 0.18, 0.25, 0.35, 0.5, 0.7, 0.85, 1.0];
const curve = [];
for (const k of KS) {
  ...
  curve.push({k, open: px.open, deep: px.deep});
}
/* invert: find k such that the OPEN pad reads what the DEEP pad reads live.
 * Both are the same concrete under the same fog, so the curve transfers. */
...
console.log(JSON.stringify({cam, time,
  n: {deep: pick.nDeep, open: pick.nOpen},
  liveWithMask: live,
  curve,
  maskFromOpenCurve: invert(live.deep, 'open'),
  maskFromDeepCurve: invert(live.deep, 'deep'),
  note: 'mask = the sun multiplier whose unshadowed pixel matches the shadowed pixel. 0 = a perfect shadow.',
  pageErrors: errs.slice(0, 5)}, null, 1));
await b.close();
