/* gx-mask2.mjs — WHY is the effective mask 0.195 where the shader's own
 * arithmetic, re-run on the CPU against the same map, says under 0.02?
 *
 * Both biases are plain uniforms, so each can be taken to zero live, on the
 * same pixels, in the same page session, and read back through the same
 * calibration ladder gx-mask.mjs established. Whatever closes the gap is the
 * cause, and it is a uniform this module writes.
 *
 *   node gx-mask2.mjs [--cam far] [--time 9]
 */
...
const pick = await page.evaluate(() => {
  ...
  for (let sy = 0; sy < innerHeight; sy += 2) for (let sx = 0; sx < innerWidth; sx += 2) {
    ...
    if (fs < 0.02 && hit) {
      ...
      /* the CORE: every neighbour 3 m away in the light plane is also fully in
       * shadow, so no amount of a metre-scale bias can walk this point out */
      let core = true;
  ...
  return {sets, par0: gi.uniforms.lemCsmParam0.value.toArray(),
...
/* calibration ladder with the mask gated off */
...
const trials = [
  ['shipped', (g) => { g.uniforms.lemCsmParam0.value.copy(g.__p0); g.uniforms.lemCsmParam1.value.copy(g.__p1); }],
  ['normalBias = 0', (g) => { g.uniforms.lemCsmParam0.value.w = 0; g.uniforms.lemCsmParam1.value.w = 0; }],
  ['normalBias x0.3', (g) => { g.uniforms.lemCsmParam0.value.w = g.__p0.w * 0.3; g.uniforms.lemCsmParam1.value.w = g.__p1.w * 0.3; }],
  ['depthBias = 0', (g) => { g.uniforms.lemCsmParam0.value.copy(g.__p0); g.uniforms.lemCsmParam1.value.copy(g.__p1);
                             g.uniforms.lemCsmParam0.value.z = 0; g.uniforms.lemCsmParam1.value.z = 0; }],
  ['both = 0', (g) => { g.uniforms.lemCsmParam0.value.z = 0; g.uniforms.lemCsmParam0.value.w = 0;
                        g.uniforms.lemCsmParam1.value.z = 0; g.uniforms.lemCsmParam1.value.w = 0; }],
  ['restored', (g) => { g.uniforms.lemCsmParam0.value.copy(g.__p0); g.uniforms.lemCsmParam1.value.copy(g.__p1); }],
];
const rows = [];
for (const [label, fn] of trials) {
  await page.evaluate(`(() => { const g = window.__lemWorld.subsystems.get('gi'); (${fn.toString()})(g); })()`);
  await page.waitForTimeout(1200);
  const px = await readPixels();
  rows.push({trial: label, ...px,
             maskDeep: invert(px.deep, 'deep'), maskCore: invert(px.deepCore, 'deepCore')});
}
