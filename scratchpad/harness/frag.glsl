...
float luminance( const in vec3 rgb ) {
	...
	return dot( weights, rgb );
        ...
        #else
          #define terrSpareDirt (vSplatB.x * 0.8 + vSplatB.y * 0.8)
...
void main() {
	...
	gl_FragDepth = vIsPerspective == 0.0 ? gl_FragCoord.z : log2( vFragDepth ) * logDepthBufFC * 0.5;
	...
	
        {
          ...
          vec3 macro = texture(tMacro, (wp - uMacroOrigin) / uMacroSize).rgb;
          float dryM = smoothstep(0.30, 0.86, macro.g);
          float turn = vSplatA.x * dryM * 0.62;
          ...
          /* Unrolled rather than looped, because every one of these fetches
           * needs an implicit derivative and a texture read inside
           * data-dependent flow has undefined LOD. Seven taps, always, is both
           * cheaper and better defined than four taps and a branch. */
          #define TERR_LAYER(I, W) { \
            vec4 s = texture(tLayers, vec3(wp / uTile[I], float(I))); \
            albedo += s.rgb * (W); \
          ...
          TERR_LAYER(0, wGrass)
          TERR_LAYER(1, vSplatA.y)
          TERR_LAYER(2, vSplatA.z + terrSpareDirt)
          {
            ...
            vec4 s = texture(tLayers, vec3(wp / uTile[3], 3.0));
            vec3 rock = s.rgb * vec3(0.76, 0.78, 0.80) + vec3(0.010, 0.012, 0.013);
            albedo += mix(s.rgb, rock, vSplatB.w) * vSplatA.w;
            bump += s.a * uBumpAmt[3] * vSplatA.w;
            rough += uRough[3] * vSplatA.w;
            poro += mix(uPoro[3], 0.26, vSplatB.w) * vSplatA.w;
          }
          #ifdef TERRAIN_FULL
            TERR_LAYER(4, vSplatB.x)
            TERR_LAYER(5, vSplatB.y)
            TERR_LAYER(6, wDry)
          #endif
          ...
          albedo *= 0.66 + macro.r * 0.62;
          float warm = macro.g - 0.5;
          albedo *= vec3(1.0 + warm * 0.20, 1.0 + warm * 0.04, 1.0 - warm * 0.24);
          float rut = macro.b * vAux.z;
          albedo *= 1.0 - rut * 0.42;
          bump += (macro.r - 0.5) * 0.55 - rut * 0.7;
          ...
          albedo = mix(albedo, uCanopyCol * (0.55 + macro.r * 0.9), vAux.y * 0.92);
          rough = mix(rough, 0.95, vAux.y);
          bump = mix(bump, bump * 0.4 + macro.r * 1.6, vAux.y);
          ...
          float puddle = clamp((vAux.x * 0.9 + rut * 0.85 + vAux.w * 0.5)
                               * smoothstep(0.04, 0.55, uWetness), 0.0, 1.0);
          puddle *= 1.0 - wGrass * 0.5;
          puddle *= 1.0 - vAux.y;
          albedo *= mix(1.0, 0.50, uWetness * poro);
          albedo = mix(albedo, albedo * 0.34, puddle);
          rough = mix(rough, rough * 0.42, uWetness * poro);
          rough = mix(rough, 0.035, puddle);
          bump = mix(bump, 0.5, puddle * 0.9);

          if (uSnow > 0.001) {
            float lie = smoothstep(0.35, 0.90, vTerrNormal.y);
            albedo = mix(albedo, vec3(0.82, 0.85, 0.90), uSnow * lie * (1.0 - puddle));
            rough = mix(rough, 0.55, uSnow * lie);
          }
...
vec3 nonPerturbedNormal = normal;
        ...
        {
          /* Surface-gradient bump (Mikkelsen). Every layer carries its height
           * in alpha, so one differentiated scalar gives all six their relief
           * and the material never touches a normal map — which is the whole
           * reason it costs seven texture reads instead of thirteen. */
          vec3 vp = -vViewPosition;
          vec3 dpdx = dFdx(vp), dpdy = dFdy(vp);
          float dhdx = dFdx(tBumpH), dhdy = dFdy(tBumpH);
          vec3 r1 = cross(dpdy, normal);
          vec3 r2 = cross(normal, dpdx);
          float det = dot(dpdx, r1);
          vec3 grad = sign(det) * (dhdx * r1 + dhdy * r2);

          /* Two guards, and the ground needs both.
           *
           * The gradient is a ratio of a height difference to a screen-space
           * position difference, and on ground seen at a grazing angle — which
           * is all of it, most of the time — the denominator collapses while
           * the numerator does not. The reconstructed normal then swings past
           * the horizon and half the fragments turn away from the sun, which
           * is what turned a grass field into black gravel in the near field.
           * Capping the perturbation at a fixed angle costs one length() and
           * is the difference between a bump map and a rash. */
          float lim = 0.7 * abs(det) / max(uBumpScale * length(grad), 1e-6);
          grad *= min(1.0, lim);

          /* And past the range where one texel is still bigger than one pixel
           * the height field has nothing true left to say, so it is faded out
           * and the macro map — comfortably above its Nyquist out there —
           * carries the surface on its own. */
          float bumpFade = 1.0 - 0.80 * smoothstep(26.0, 115.0, length(vViewPosition));
...
IncidentLight directLight;
	...
	PointLightShadow pointLightShadow;
		...
		directLight.color *= ( directLight.visible && receiveShadow ) ? getPointShadow( pointShadowMap[ 1 ], pointLightShadow.shadowMapSize, pointLightShadow.shadowIntensity, pointLightShadow.shadowBias, pointLightShadow.shadowRadius, vPointShadowCoord[ 1 ], pointLightShadow.shadowCameraNear, pointLightShadow.shadowCameraFar ) : 1.0;
		#endif
		RE_Direct( directLight, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, material, reflectedLight );
		...
		directLight.color *= ( directLight.visible && receiveShadow ) ? getPointShadow( pointShadowMap[ 2 ], pointLightShadow.shadowMapSize, pointLightShadow.shadowIntensity, pointLightShadow.shadowBias, pointLightShadow.shadowRadius, vPointShadowCoord[ 2 ], pointLightShadow.shadowCameraNear, pointLightShadow.shadowCameraFar ) : 1.0;
		#endif
		RE_Direct( directLight, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, material, reflectedLight );
	
		pointLight = pointLights[ 3 ];
		getPointLightInfo( pointLight, geometryPosition, directLight );
		#if defined( USE_SHADOWMAP ) && ( 3 < 0 )
		pointLightShadow = pointLightShadows[ 3 ];
		directLight.color *= ( directLight.visible && receiveShadow ) ? getPointShadow( pointShadowMap[ 3 ], pointLightShadow.shadowMapSize, pointLightShadow.shadowIntensity, pointLightShadow.shadowBias, pointLightShadow.shadowRadius, vPointShadowCoord[ 3 ], pointLightShadow.shadowCameraNear, pointLightShadow.shadowCameraFar ) : 1.0;
		#endif
		RE_Direct( directLight, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, material, reflectedLight );
	
		pointLight = pointLights[ 4 ];
		getPointLightInfo( pointLight, geometryPosition, directLight );
		#if defined( USE_SHADOWMAP ) && ( 4 < 0 )
		pointLightShadow = pointLightShadows[ 4 ];
		directLight.color *= ( directLight.visible && receiveShadow ) ? getPointShadow( pointShadowMap[ 4 ], pointLightShadow.shadowMapSize, pointLightShadow.shadowIntensity, pointLightShadow.shadowBias, pointLightShadow.shadowRadius, vPointShadowCoord[ 4 ], pointLightShadow.shadowCameraNear, pointLightShadow.shadowCameraFar ) : 1.0;
		#endif
		RE_Direct( directLight, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, material, reflectedLight );
	
		pointLight = pointLights[ 5 ];
		getPointLightInfo( pointLight, geometryPosition, directLight );
		#if defined( USE_SHADOWMAP ) && ( 5 < 0 )
		pointLightShadow = pointLightShadows[ 5 ];
		directLight.color *= ( directLight.visible && receiveShadow ) ? getPointShadow( pointShadowMap[ 5 ], pointLightShadow.shadowMapSize, pointLightShadow.shadowIntensity, pointLightShadow.shadowBias, pointLightShadow.shadowRadius, vPointShadowCoord[ 5 ], pointLightShadow.shadowCameraNear, pointLightShadow.shadowCameraFar ) : 1.0;
		#endif
		RE_Direct( directLight, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, material, reflectedLight );
	
		pointLight = pointLights[ 6 ];
		getPointLightInfo( pointLight, geometryPosition, directLight );
		#if defined( USE_SHADOWMAP ) && ( 6 < 0 )
		pointLightShadow = pointLightShadows[ 6 ];
		directLight.color *= ( directLight.visible && receiveShadow ) ? getPointShadow( pointShadowMap[ 6 ], pointLightShadow.shadowMapSize, pointLightShadow.shadowIntensity, pointLightShadow.shadowBias, pointLightShadow.shadowRadius, vPointShadowCoord[ 6 ], pointLightShadow.shadowCameraNear, pointLightShadow.shadowCameraFar ) : 1.0;
		#endif
		RE_Direct( directLight, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, material, reflectedLight );
	
		pointLight = pointLights[ 7 ];
		getPointLightInfo( pointLight, geometryPosition, directLight );
		#if defined( USE_SHADOWMAP ) && ( 7 < 0 )
		pointLightShadow = pointLightShadows[ 7 ];
		directLight.color *= ( directLight.visible && receiveShadow ) ? getPointShadow( pointShadowMap[ 7 ], pointLightShadow.shadowMapSize, pointLightShadow.shadowIntensity, pointLightShadow.shadowBias, pointLightShadow.shadowRadius, vPointShadowCoord[ 7 ], pointLightShadow.shadowCameraNear, pointLightShadow.shadowCameraFar ) : 1.0;
		#endif
		RE_Direct( directLight, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, material, reflectedLight );
	
		pointLight = pointLights[ 8 ];
		getPointLightInfo( pointLight, geometryPosition, directLight );
		#if defined( USE_SHADOWMAP ) && ( 8 < 0 )
		pointLightShadow = pointLightShadows[ 8 ];
		directLight.color *= ( directLight.visible && receiveShadow ) ? getPointShadow( pointShadowMap[ 8 ], pointLightShadow.shadowMapSize, pointLightShadow.shadowIntensity, pointLightShadow.shadowBias, pointLightShadow.shadowRadius, vPointShadowCoord[ 8 ], pointLightShadow.shadowCameraNear, pointLightShadow.shadowCameraFar ) : 1.0;
		#endif
		RE_Direct( directLight, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, material, reflectedLight );
	
		pointLight = pointLights[ 9 ];
		getPointLightInfo( pointLight, geometryPosition, directLight );
		#if defined( USE_SHADOWMAP ) && ( 9 < 0 )
		pointLightShadow = pointLightShadows[ 9 ];
		directLight.color *= ( directLight.visible && receiveShadow ) ? getPointShadow( pointShadowMap[ 9 ], pointLightShadow.shadowMapSize, pointLightShadow.shadowIntensity, pointLightShadow.shadowBias, pointLightShadow.shadowRadius, vPointShadowCoord[ 9 ], pointLightShadow.shadowCameraNear, pointLightShadow.shadowCameraFar ) : 1.0;
		#endif
		RE_Direct( directLight, geometryPosition, geometryNormal, geometryViewDir, geometryClearcoatNormal, material, reflectedLight );
  ...
  #if defined( RE_IndirectDiffuse )
    irradiance += lemIndirect( vLemWorld,
  ...
  #ifdef LEM_SSAO
    float lemAO = texture2D( lemAOMap, gl_FragCoord.xy / lemAORes ).x;
    lemAO = clamp( mix( 1.0, lemAO, lemAOStrength ), 0.0, 1.0 );
...
#ifdef USE_FOG
  #ifdef FOG_EXP2
    float lemH0 = cameraPosition.y - 0.0;
    float lemH1 = vFogHeight - 0.0;
    float lemA = exp( - max( lemH0, -600.0 ) / 130.0 );
    float lemB = exp( - max( lemH1, -600.0 ) / 130.0 );
    float lemDy = lemH1 - lemH0;
    float lemAvg = abs( lemDy ) < 1.0
      ? 0.5 * ( lemA + lemB )
      : 130.0 * ( lemA - lemB ) / lemDy;
    float lemTau = fogDensity * vFogDepth * clamp( lemAvg, 0.0, 6.0 );
    float fogFactor = 0.860 * ( 1.0 - exp( - lemTau * lemTau ) );
  #else
