...
/* rz-near: tv-near.mjs with the passing loops FORCED ON, because trains.js does
 * not consume `variants`. `rail.cycle` is wrapped so it hands back a variant —
 * a crossover exit — instead of the full lap, alternating per call, so some
 * workings take a crossover and some take the full lap and the two end up level
 * with each other at a throat. That is the pair the collision was found in.
 *
 * It has to be installed BEFORE `setMachines`, because `_stand` seats every
 * consist on to its cycle there and a consist re-seated afterwards loses its
 * reservation: a first attempt patched after seating and produced two workings
 * at identical arc length with `holds: []`, which is the harness breaking the
 * train, not the railway. */
await p.evaluate(() => {
  const rail = window.__lemWorld.subsystems.get('rail');
  const raw = rail.cycle.bind(rail);
  let n = 0;
  window.__loopStats = {calls: 0, variantsUsed: 0, lines: {}};
  rail.cycle = uid => {
    const base = raw(uid);
    window.__loopStats.calls++;
    const vs = base && base.variants;
    if (!vs || vs.length < 2) return base;
    const pick = vs[n++ % vs.length];
    if (!pick || pick === base) return base;
    window.__loopStats.variantsUsed++;
    window.__loopStats.lines[pick.line] = (window.__loopStats.lines[pick.line] || 0) + 1;
...
console.log(JSON.stringify(await p.evaluate(() => {
  ...
  return {loopStats: window.__loopStats, hits: window.__hits};
}), null, 1));
await b.close();
