...
class MasterCSVWriter:
    ...
    def _worker(cls, master_file: str, queue: Queue) -> None:
        while True:
            ...
            try:
                ...
                log_message('error', f"Failed to append to '{master_file}': {exc}")
            finally:
                queue.task_done()
    ...
    def _write(master_file: str, df_chunk: pd.DataFrame, machine_name: str) -> None:
        ...
        log_message('info', f"Appended {len(df_chunk)} row(s) from {machine_name} to '{master_file}'.")
...
def _process_multi_csv_file(csv_path: str, move_folder: str, config_folder: str) -> None:
    try:
        ...
        log_message('error', f"Error processing multi CSV '{csv_path}': {exc}")
    finally:
