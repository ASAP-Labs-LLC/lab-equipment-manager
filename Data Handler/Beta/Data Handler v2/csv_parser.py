...
def invalidate_config_cache(config_folder: str = None) -> None:
    """
    Force-invalidate the config cache so the next load_parser_config() reads
    fresh data from disk.  Pass a specific config_folder to invalidate one
    entry, or None to clear the entire cache.
    """
    with _CONFIG_CACHE_LOCK:
        if config_folder is None:
            _CONFIG_CACHE.clear()
            log_message('debug', "Invalidated entire config cache.")
        else:
            config_file = os.path.join(
                os.path.abspath(config_folder), 'parser_config.json'
            )
            cache_key = os.path.normcase(config_file)
            if _CONFIG_CACHE.pop(cache_key, None) is not None:
                log_message('debug', f"Invalidated config cache for '{config_folder}'.")
...
def parse_csv(file_like_object, config_folder, delimiter=','):
    ...
    try:
        ...
        # Track input row count for reconciliation
        input_row_count = len(data)
        log_message('debug', f"Input row count after CSV read: {input_row_count}")
        ...
        # 10) Row count reconciliation — detect silent data loss
        output_row_count = len(df)
        if output_row_count == 0 and input_row_count > 0:
            log_message('error',
                f"DATA INTEGRITY: {input_row_count} input row(s) produced 0 output rows! "
                f"Check parser config for '{config_folder}'."
            )
        elif output_row_count != input_row_count:
            log_message('warning',
                f"Row count changed during parsing: {input_row_count} input -> "
                f"{output_row_count} output for '{config_folder}'."
            )
        else:
            log_message('debug',
                f"Row count verified: {input_row_count} in -> {output_row_count} out."
            )

        # 11) Column integrity — verify expected columns are present
        expected_cols = set(header_columns)
        actual_cols = set(df.columns)
        missing_cols = expected_cols - actual_cols
        if missing_cols:
            log_message('warning',
                f"DATA INTEGRITY: Expected columns missing from output: {missing_cols}"
            )

        # 12) Check for all-empty rows that might indicate a parse failure
        empty_rows = df.apply(lambda row: all(str(v).strip() == '' for v in row), axis=1).sum()
        if empty_rows > 0 and empty_rows == output_row_count:
            log_message('error',
                f"DATA INTEGRITY: All {output_row_count} output row(s) are empty! "
                f"Parsing may have failed silently for '{config_folder}'."
            )
            return pd.DataFrame()  # Return empty to prevent writing garbage

        # 13) Log final DataFrame to parsed_data.log
        ...
        log_message('info', f"Parsed CSV data successfully: {output_row_count} row(s), {len(df.columns)} col(s).")
        return df
...
def apply_correction_factors(df, output_destination, config):
    ...
    """
    if not config.get('correction_factors_enabled', True):
        log_message('debug', "Correction factors disabled for this parser. Skipping.")
        return df
...
def apply_math_operations(df, config):
    ...
    for operation in operations:
        ...
        if match:
            ...
            try:
                ...
                failed_ops.append(sanitized_op)
    ...
    if failed_ops:
        log_message('warning',
            f"DATA INTEGRITY: {len(failed_ops)}/{len(operations)} math operation(s) failed: "
            f"{failed_ops}"
        )
