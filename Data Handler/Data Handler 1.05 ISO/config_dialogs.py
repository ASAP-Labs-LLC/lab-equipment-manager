...
def _enumerate_serial_ports():
    """Return available serial port device names across Windows/Mac/Linux.

    Uses pyserial's cross-platform discovery: on Windows yields names like
    'COM3', on macOS '/dev/tty.usbserial-*' / '/dev/tty.usbmodem*', on Linux
    '/dev/ttyUSB*' / '/dev/ttyACM*'.
    """
    if _serial_list_ports is None:
        return []
    try:
        return sorted({p.device for p in _serial_list_ports.comports() if p.device})
    except Exception:
        return []
...
class ConfigDialog(QDialog):
    ...
    def update_parser_type_fields(self, parser_type):
        ...
        if parser_type == "single":
            ...
            # Editable combo populated via serial.tools.list_ports.comports() so
            # port names are correct on Windows (COM3), macOS (/dev/tty.usbserial-*),
            # and Linux (/dev/ttyUSB*). Users may still type a custom value.
            self.port_input = QComboBox()
            self.port_input.setEditable(True)
            discovered_ports = _enumerate_serial_ports()
            for dev in discovered_ports:
                self.port_input.addItem(dev)
            if port_val and port_val not in discovered_ports:
                self.port_input.addItem(port_val)
            if port_val:
                self.port_input.setCurrentText(port_val)
            self.refresh_ports_button = QPushButton("Refresh")
            self.refresh_ports_button.clicked.connect(self._refresh_port_list)
            ...
            form_layout.addRow("COM Port:", port_row)
    ...
    def save_config(self):
        ...
        if parser_type == "single":
            ...
            # port_input is an editable QComboBox; use currentText() rather than text()
