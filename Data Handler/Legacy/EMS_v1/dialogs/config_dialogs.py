# File: csv_parser_app/dialogs/config_dialogs.py
...
from PyQt5.QtWidgets import (
    ...
    QFileDialog, QMessageBox, QFormLayout, QCheckBox
)
from PyQt5.QtCore import Qt
from typing import Dict, Optional, List
from .advanced_com_settings_dialog import AdvancedCOMSettingsDialog
from .data_actions_dialog import DataActionsDialog
...
class ConfigDialog(QDialog):
    ...
    def __init__(self, existing_config: Optional[Dict] = None):
        super().__init__()
    ...
    def create_file_selector(self, label_text: str):
        layout = QHBoxLayout()
    ...
    def create_directory_selector(self, label_text: str):
        layout = QHBoxLayout()
        label = QLabel(label_text)
        line_edit = QLineEdit()
        browse_button = QPushButton("Browse")
    ...
    def update_parser_type_fields(self, parser_type: str) -> None:
    ...
    def open_advanced_com_settings(self) -> None:
        existing_settings = self.config.setdefault('COM', {})
        from .advanced_com_settings_dialog import AdvancedCOMSettingsDialog
        dialog = AdvancedCOMSettingsDialog(existing_settings)
        if dialog.exec_():
            self.config['COM'].update(dialog.get_settings())

    def edit_data_actions(self) -> None:
    ...
    def save_config(self) -> None:
        ...
        if parser_type == "single":
            # Validate and save single CSV parser settings
            ...
            self.config['single_csv'] = {
                ...
                'output': output_csv,
                'delimiter': delimiter,
                'last_position': 0  # Reset last_position
            }

        elif parser_type == "multi":
            # Validate and save multi CSV parser settings
            ...
            # Validate and save COM parser settings
