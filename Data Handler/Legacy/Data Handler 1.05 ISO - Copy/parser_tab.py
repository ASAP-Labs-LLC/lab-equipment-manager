...
from csv_parser import append_raw_suffix
...
class ParserTab(QWidget):
    ...
    def __init__(self, config):
        ...
        if parser_type == 'single':
            self.parsed_csv = self.config['single_csv']['output']
        elif parser_type == 'multi':
            self.parsed_csv = self.config['multi']['output']
        elif parser_type == 'COM':
            self.parsed_csv = self.config['COM']['output']
        else:
            self.parsed_csv = os.path.join(self.config['config_folder'], 'output.csv')
