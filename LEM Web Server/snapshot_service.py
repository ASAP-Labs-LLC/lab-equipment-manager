...
class SnapshotService:
    ...
    def ensure_schema(self) -> None:
        ...
        for ddl in SCHEMA_DDL:
            ...
            # single boot. That is precisely the cost `_existing_tables` exists
            # to avoid, and the tray restarts this server on every code edit.
            if "CREATE INDEX" in ddl.upper():
                index = ddl.split("IF NOT EXISTS", 1)[1].split(" ON ", 1)[0]
                if indexes is not None and index.strip() in indexes:
                    continue
            else:
