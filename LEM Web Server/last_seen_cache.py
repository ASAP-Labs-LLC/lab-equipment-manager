...
def build_evaluation_from_entry(entry: Dict[str, Any],
    ...
    if status in (STATUS_GREEN, STATUS_YELLOW):
        ...
        if anchor:
            # Same rolling window as data_source.evaluate_box — a cached verdict
            # must not expire on a different rule from a live one.
            from data_source import qc_is_stale
            if qc_is_stale(anchor, NOW(), box.qc_expire_hours):
