...
class LastSeenCache:
    ...
    def sync_config(self, cfg: AppConfig) -> None:
        ...
        # Update watcher signatures and check for path changes
        for box in cfg.boxes:
            entry = boxes.get(box.uid)
            if not entry:
                continue
            
            # If CSV path changed, invalidate cache for this box
            # This ensures we don't show stale data from the old file
            if entry.get("csv_path") != box.csv_path:
...
def build_evaluation_from_entry(entry: Dict[str, Any],
    ...
    if status in (STATUS_GREEN, STATUS_YELLOW):
        ...
        if anchor:
            age = NOW() - anchor
            # Calendar-day boundary: convert stored hours to days, expire at day boundary
            expire_days = max(1, int(round(box.qc_expire_hours / 24.0)))
            qc_expiry_date = anchor.date() + timedelta(days=expire_days)
            if qc_expiry_date <= NOW().date():
