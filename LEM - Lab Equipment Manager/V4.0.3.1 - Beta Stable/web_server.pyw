...
Run:
    python web_server.py --host 0.0.0.0 --port 5557
Environment:
...
from platform_utils import IS_WINDOWS, IS_MAC, IS_LINUX
from models import (
    AppConfig,
    BoxConfig,
    ChecklistItem,
...
# These are deployment-specific constants; adjust per site/OS as needed.
# - LABMGR_FS_ROOTS env var (os.pathsep-separated) overrides the defaults
#   so the same code works on Windows, macOS, and Linux without edits.
# - Windows defaults include the original lab share (UNC) and common drives.
# - macOS defaults include the SMB mount point (/Volumes/Labsharedrive) and HOME.
# - Linux defaults fall back to /mnt, /media, and HOME.
_WINDOWS_DEFAULT_ROOTS = [
    "//asapserver/Labsharedrive",  # original lab UNC share
    "C:/",
    "D:/",
]
_MAC_DEFAULT_ROOTS = [
    "/Volumes/Labsharedrive",      # SMB mount of the lab share on macOS
    "/Volumes",
    os.path.expanduser("~"),
]
_LINUX_DEFAULT_ROOTS = [
    "/mnt",
    "/media",
    os.path.expanduser("~"),
]
...
def _default_allowed_roots() -> List[str]:
    env = os.environ.get("LABMGR_FS_ROOTS", "").strip()
    if env:
        return [p for p in env.split(os.pathsep) if p]
    if IS_WINDOWS:
        return list(_WINDOWS_DEFAULT_ROOTS)
    if IS_MAC:
        return list(_MAC_DEFAULT_ROOTS)
    return list(_LINUX_DEFAULT_ROOTS)
...
def log_status_event(cfg: AppConfig, category: str, box: BoxConfig, prev: str, curr: str, reason: str, context: str = "") -> None:
    """Generic status change logger for Overall, QC, PM, Calibration."""
    out_dir = (cfg.status_log_dir or "").strip()
    if not out_dir:
        return
    try:
        # Determine sub-folder and filename based on category
        # Categories: OVERALL, QC, PM, CALIBRATION
        sub_dir = os.path.join(out_dir, "Equipment")
        os.makedirs(sub_dir, exist_ok=True)
        
        filename = f"{category}_Status_Changes.csv"
        ...
        with open(path, "a", newline="", encoding="utf-8") as f:
            ...
            if not file_exists:
                # Common headers
                headers = ["timestamp", "box_uid", "box_title", "prev_status", "new_status", "reason"]
                if category == "QC":
            ...
            if category == "QC":
                row.append(context or "")
            writer.writerow(row)
    except Exception as e:
        print(f"Logging failed for {category}: {e}")

def log_maintenance_activity(cfg: AppConfig, category: str, box_uid: str, task_name: str, action: str, user: str, notes: str, next_due: str = "") -> None:
    """Log PM/Cal activities."""
    out_dir = (cfg.status_log_dir or "").strip()
    if not out_dir: return
    try:
        sub_dir = os.path.join(out_dir, "Maintenance")
        os.makedirs(sub_dir, exist_ok=True)
        filename = f"{category}_Activity_Log.csv" # PM or Calibration
        path = os.path.join(sub_dir, filename)
        ...
        with open(path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            if not file_exists:
                writer.writerow(["timestamp", "box_uid", "task_name", "action_type", "user", "notes", "next_due"])
...
def log_inventory_event(cfg: AppConfig, action: str, box: BoxConfig, user: str) -> None:
    """Log creation/deletion of equipment."""
    out_dir = (cfg.status_log_dir or "").strip()
    if not out_dir: return
    try:
        sub_dir = os.path.join(out_dir, "Configuration")
        os.makedirs(sub_dir, exist_ok=True)
        path = os.path.join(sub_dir, "Equipment_Inventory_Log.csv")
        
        file_exists = os.path.exists(path)
        with open(path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            if not file_exists:
                writer.writerow(["timestamp", "action", "box_uid", "box_title", "user"])
...
def log_checklist_event(cfg: AppConfig, checklist_name: str, item_text: str, action: str, user: str, status: str) -> None:
    """Log checklist interactions."""
    out_dir = (cfg.status_log_dir or "").strip()
    if not out_dir: return
    try:
        sub_dir = os.path.join(out_dir, "Checklists")
        os.makedirs(sub_dir, exist_ok=True)
        path = os.path.join(sub_dir, "Checklist_Activity_Log.csv")
        
        file_exists = os.path.exists(path)
        with open(path, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            if not file_exists:
                writer.writerow(["timestamp", "checklist_name", "item_text", "action", "user", "completion_status"])
...
class StatusEngine:
    def __init__(self) -> None:
        ...
        self.cache = LastSeenCache(DATA_DIR)
        self.cache.sync_config(self.cfg)
        self.maintenance = MaintenanceManager(DATA_DIR)
        self._sync_maintenance_dirs()
        self._sync_maintenance_dirs()
        # _last_status maps UID -> { overall: str, qc: str, pm: str, calibration: str }
    ...
    def _load_csv_rows(self, paths: Iterable[str]) -> Tuple[Dict[str, List[dict]], Dict[str, str]]:
        ...
        for path in paths:
            ...
            try:
                ...
                rows[path] = parsed
                self._csv_cache[path] = {"mtime": mtime, "size": size, "rows": parsed}
            except Exception as exc:
                rows[path] = []
                # Do NOT cache failure - force retry next loop
    ...
    def _export_daily_logs(self, today_str: str, report_dir: str):
        ...
        # 3. Correction Factor Log
        # Read from correction_factor_dir/correction_factor_changes.csv
        cf_dir = (self.cfg.correction_factor_dir or "").strip()
        if cf_dir:
            cf_log = os.path.join(cf_dir, "correction_factor_changes.csv")
            if os.path.exists(cf_log):
                try:
                    dst = os.path.join(report_dir, f"CorrectionFactors_{today_str}.csv")
                    with open(cf_log, "r", encoding="utf-8") as fin:
                        reader = csv.reader(fin)
                        header = next(reader, None)
                        rows = []
                        if header:
                    ...
                    print(f"Correction factor log export failed: {e}")
    ...
    def refresh(self, force: bool = False) -> None:
        ...
        for box in cfg.boxes:
            ...
            # Get previous states (default to UNKNOWN if first run)
            last_states = self._last_status.get(box.uid, {
                "overall": STATUS_UNKNOWN, "qc": STATUS_UNKNOWN, "pm": STATUS_UNKNOWN, "calibration": STATUS_UNKNOWN
            })
            
            # 1. Overall Status
            if last_states["overall"] != status:
                log_status_event(cfg, "Overall", box, last_states["overall"], status, reason)
                last_states["overall"] = status

            # Extract sub-statuses from evaluation
            subs = getattr(evaluation, "sub_statuses", {})
            
            # 2. QC Status
            qc_s = subs.get("qc", {}).get("status", STATUS_UNKNOWN)
            if last_states["qc"] != qc_s:
                # For QC, reason might be specific context failure?
                # We'll use the generic explanation or try to dig it out.
                qc_reason = subs.get("qc", {}).get("reason", "")
                log_status_event(cfg, "QC", box, last_states["qc"], qc_s, qc_reason)
                last_states["qc"] = qc_s

            # 3. PM Status
            pm_s = subs.get("pm", {}).get("status", STATUS_UNKNOWN)
            if last_states["pm"] != pm_s:
                pm_reason = subs.get("pm", {}).get("reason", "")
                log_status_event(cfg, "PM", box, last_states["pm"], pm_s, pm_reason)
                last_states["pm"] = pm_s

            # 4. Calibration Status
            cal_s = subs.get("calibration", {}).get("status", STATUS_UNKNOWN)
            if last_states["calibration"] != cal_s:
                cal_reason = subs.get("calibration", {}).get("reason", "")
                log_status_event(cfg, "Calibration", box, last_states["calibration"], cal_s, cal_reason)
                last_states["calibration"] = cal_s

            self._last_status[box.uid] = last_states
...
CHECKLIST_STATE_FILE = os.path.join(DATA_DIR, "checklist_state.json")
...
def api_toggle_checklist_item():
    ...
    with CHECKLIST_LOCK:
        ...
        for checklist_uid, item_key in affected_items:
            ...
            # Log Event
            try:
                cl_obj = next((c for c in cfg.checklists if c.uid == checklist_uid), None)
                if cl_obj:
                    it_obj = next((i for i in cl_obj.items if i.uid == item_key), None)
                    txt = it_obj.text if it_obj else str(item_key)
                    log_checklist_event(cfg, cl_obj.name, txt, "CHECKED" if checked else "UNCHECKED", user, "INCOMPLETE") 
            except:
                pass
    ...
    # Prepare response data (same as api_get_checklists)
    state = _get_today_state()
    serial_state = {}
    for (c_uid, i_key), val in state.items():
        serial_state[f"{c_uid}_{i_key}"] = val
    ...
    BUS.publish({"type": "checklist", "data": response_data})
...
def _log_correction_change(log_path: str, machine_name: str, test_name: str,
                           ...
                           prev_value: float, new_value: float, user: str,
...
def api_save_correction(uid: str):
    ...
    if log_path:
        _log_correction_change(
            ...
            box.csv_path, prev_value, correction_value, get_current_user()
        )
...
def api_delete_correction(uid: str, test_name: str):
    ...
    if log_path:
        _log_correction_change(
            log_path, machine_name, test_name, value_column,
            box.csv_path, prev_value, 0.0, get_current_user()
        )
...
def api_create_task():
    ...
    if tpl:
        ENGINE.refresh(force=True)
        log_maintenance_activity(ENGINE.get_config(), tpl.kind.upper(), tpl.box_uid, tpl.name, "CREATED", get_current_user(), f"Repeat: {repeat_value} {repeat_unit}", tpl.next_due)
...
def api_start_task(task_id: str):
    ...
    if tpl:
        log_maintenance_activity(ENGINE.get_config(), tpl.kind.upper(), tpl.box_uid, tpl.name, "STARTED", get_current_user(), "")
    ENGINE.refresh(force=True)
...
def api_complete_task(task_id: str):
    ...
    if tpl:
        ...
        log_maintenance_activity(ENGINE.get_config(), tpl.kind.upper(), tpl.box_uid, tpl.name, "COMPLETED", user, comment, tpl.next_due)
    ENGINE.refresh(force=True)
    return jsonify({"ok": bool(tpl)})
...
def api_delete_task(task_id: str):
    ...
    tpl = ENGINE.maintenance.templates.get(task_id)
    success = ENGINE.maintenance.delete_task(task_id)
    if success and tpl:
        log_maintenance_activity(ENGINE.get_config(), tpl.kind.upper(), tpl.box_uid, tpl.name, "DELETED", get_current_user(), "")
    ENGINE.refresh(force=True)
...
def build_report_rows(cfg: AppConfig,
    ...
    # Cleaner, human-readable headers
    headers = [
        "Equipment Name", "Overall Status", "Manual Override",
        "Last Good QC", "Latest Data Time",
        "QC Expiry (days)", "Status Reason",
        "Sample", "Test", "Expected Value", "Tolerance (±)", "Low Limit", "High Limit",
        "Latest Value", "Units", "In Spec", "Data Source",
    ]
    ...
    for box in cfg.boxes:
        ...
        last_qc = ev.last_good_qc.strftime("%Y-%m-%d %H:%M") if ev.last_good_qc else "—"
        latest_match = ev.latest_match_time.strftime("%Y-%m-%d %H:%M") if ev.latest_match_time else "—"
        override_label = box.manual_override if box.manual_override else "None"
        if ev.results:
            for pr in ev.results:
                ...
                if test:
                    ...
                    low = f"{pr.low:.4g}" if pr.low is not None else "—"
                    high = f"{pr.high:.4g}" if pr.high is not None else "—"
                    expected = f"{test.expected:.4g}"
                    latest = "—" if pr.latest_value is None else f"{pr.latest_value:.4g}"
                    insp = "—" if pr.in_spec is None else ("✓ PASS" if pr.in_spec else "✗ FAIL")
                    ts_source = getattr(pr, "timestamp_source", "")
                    out_rows.append([
                        box.title, status, override_label,
                        last_qc, latest_match,
                        f"{max(1, round(box.qc_expire_hours / 24))}", reason,
                        sample_name, test.name, expected, f"{tol:.4g}",
                        low, high, latest, test.units, insp, ts_source,
                    ])
                else:
                    out_rows.append([
                        box.title, status, override_label,
            ...
            out_rows.append([
                box.title, status, override_label,
                last_qc, latest_match,
                f"{box.qc_expire_hours:.0f}", reason,
                "—", "—", "—", "—", "—", "—", "—", "—", "—", "—",
            ])
...
def api_report_preview():
    ...
    if request.args.get("download") == "1":
        out = os.path.join(DATA_DIR, "report_preview.csv")
...
@app.get("/api/history")
def api_history():
    """Return all historical log data for the History tab."""
    cfg = ENGINE.get_config()
    base = (cfg.status_log_dir or "").strip()

    status_path = os.path.join(base, "status_changes.csv") if base else os.path.join(DATA_DIR, "status_changes.csv")
    manual_path = os.path.join(base, "manual_overrides.csv") if base else os.path.join(DATA_DIR, "manual_overrides.csv")

    status_rows = read_log(status_path)
    manual_rows = read_log(manual_path)

    # Maintenance log from in-memory records
    maint_rows = []
    for e in ENGINE.maintenance.log:
        row = e.__dict__.copy()
        row.pop("_sa_instance_state", None)
        maint_rows.append(row)
    # Sort descending by timestamp
    maint_rows.sort(key=lambda r: r.get("timestamp", ""), reverse=True)

    # Checklist daily completion summary from CHECKLIST_STATE
    with CHECKLIST_LOCK:
        state_snap = {k: dict(v) for k, v in CHECKLIST_STATE.items()}

    checklist_summary = []
    for date_str in sorted(state_snap.keys(), reverse=True):
        items = state_snap[date_str]
        total = 0
        checked = 0
        for val in items.values():
            if isinstance(val, dict) and "checked" in val:
                total += 1
                if val.get("checked"):
                    checked += 1
    ...
    return jsonify({
        ...
        "maintenance_log": maint_rows,
        "checklist_summary": checklist_summary,
    })


@app.get("/api/boxes/<box_uid>/qc-data")
def api_box_qc_data(box_uid: str):
    """Return recent raw CSV rows + metadata for QC trend charting."""
    cfg = ENGINE.get_config()
    box = next((b for b in cfg.boxes if b.uid == box_uid), None)
    if not box or not box.csv_path:
        return jsonify({"error": "Box not found or no CSV path"}), 404

    samples_by_name = {s.name: s for s in cfg.samples}

    sample_id_column = cfg.sample_id_column or "Lab ID"

    # Resolve value_col for each watched target
    watched_info = []
    for wt in box.watched_targets:
        sample = samples_by_name.get(wt.sample)
        if not sample:
            continue
        for ts in sample.tests:
            if ts.name == wt.test:
                watched_info.append({
                    "label": f"{wt.sample} / {wt.test}",
                    "value_col": ts.value_col,
                    "expected": ts.expected,
                    "low": ts.expected - ts.k * ts.std_dev,
                    "high": ts.expected + ts.k * ts.std_dev,
                    "units": ts.units,
                    "sample_id_val": sample.sample_id_val,
                })
                break

    try:
        rows = read_csv_rows(box.csv_path)
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500

    # Return last 500 rows (sufficient for a trend chart)
    recent = rows[-500:] if len(rows) > 500 else rows

    return jsonify({
        "timestamp_col": box.timestamp_col or "",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--port", type=int, default=5557, help="Port to listen on (default: 5557)")
...
def _create_tray_icon():
    ...
    try:
        ...
        # Green circle with dark border - represents "server running"
        draw.ellipse([4, 4, 60, 60], fill="#21c071", outline="#0f172a", width=3)
        # "L" letter in center
        draw.text((22, 14), "L", fill="white")
        return img
    except Exception:
        # Fallback: plain green square
        from PIL import Image
        return Image.new("RGB", (64, 64), "#21c071")


def _start_server(host: str, port: int) -> None:
    """Start the Flask server and StatusEngine in a background thread."""
    global ENGINE
    ENGINE = StatusEngine()
    # Use debug=False when running under tray (debug=True spawns reloader subprocess)
    app.run(host=host, port=port, threaded=True, debug=False, use_reloader=False)


def _run_with_tray(args: argparse.Namespace) -> None:
    """Run the web server with a system tray icon (cross-platform via pystray)."""
    import pystray
    ...
    server_thread = threading.Thread(
        target=_start_server, args=(host, port), daemon=True, name="flask-server"
    )
    server_thread.start()
    ...
    def on_restart(icon, item):
        """Restart by spawning a new process and exiting this one."""
        icon.stop()
        try:
            ENGINE.shutdown()
        except Exception:
            pass
        # Re-launch the same script with same arguments.
        # CREATE_NEW_PROCESS_GROUP is Windows-only; on POSIX use start_new_session
        # so Ctrl+C in the parent doesn't kill the relaunched child.
        popen_kwargs = {}
    ...
    menu = pystray.Menu(
        pystray.MenuItem(f"LEM Server - port {port}", None, enabled=False),
    ...
    icon = pystray.Icon("LEM Server", _create_tray_icon(), "LEM Server", menu)
    print(f"LEM Server running at {url}")
...
def main() -> None:
    ...
    print("\n")
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print("!!!  LEM Lab Equipment Manager - Web Server          !!!")
    print(f"!!!  Port: {args.port:<45}!!!")
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print()

    # Try to run with system tray; fall back to console-only if pystray unavailable
    try:
