...
def evaluate_box(box: BoxConfig,
    ...
    if test_status == STATUS_GREEN and last_good_qc:
        # Calendar-day boundary: as soon as it becomes a new calendar day, QC must be retested.
        # qc_expire_hours is stored in hours for backward compat; convert to calendar days.
        expire_days = max(1, int(round(box.qc_expire_hours / 24.0)))
        qc_expiry_date = last_good_qc.date() + timedelta(days=expire_days)
