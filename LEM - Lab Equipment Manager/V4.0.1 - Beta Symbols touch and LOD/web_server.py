...
LOCAL_SHARE_ROOT = None

if IS_LINUX:
    # Attempt to locate the local mount point of the share.
    # We assume 'web_server.py' is running from within the share.
    # Current path: .../share=labsharedrive/Ryan C/... or similar GVFS path
    # We want to map //asapserver/Labsharedrive -> /run/.../share=labsharedrive
    
    _current = pathlib.Path(BASE_DIR).resolve()
    # Iterate parents to find the "root" of the share. 
    # Heuristic: look for 'labsharedrive' in path parts (case-insensitive)
    for _p in [_current] + list(_current.parents):
        if "labsharedrive" in _p.name.lower():
             LOCAL_SHARE_ROOT = _p
             print(f"Detected Local Share Root: {LOCAL_SHARE_ROOT}")
             break
    
    # Fallback: if not found, maybe we are just in the root?
    if not LOCAL_SHARE_ROOT:
         # Just use current directory as a fallback?
         pass

def resolve_path(path: str) -> str:
    """
    Resolve a Windows UNC path (//asapserver/Labsharedrive/...) to the local
    Linux GVFS mount path if applicable. 
    Also normalizes OS separators.
    """
    if not path:
        return ""
    
    # 1. Normalize slashes for processing (make everything forward slash temporarily)
    p = path.replace("\\", "/")
    
    # 2. Check for Windows UNC prefix if we are on Linux and have a mapped root
    if IS_LINUX and LOCAL_SHARE_ROOT:
        # UNC Target: //asapserver/Labsharedrive
        # Note: input might handle // as just / in some normalizations, but let's check string
        unc_marker = "//asapserver/labsharedrive"
        if p.lower().startswith(unc_marker):
            # Strip prefix
            rel = p[len(unc_marker):]
            if rel.startswith("/"):
                rel = rel[1:]
            
            # Reconstruct
            return str(LOCAL_SHARE_ROOT / rel)
            
        # Also handle "S:/" mapped drives if they appear (just in case)
        # e.g. S:/ASAP Lab Results...
        # But config uses UNC usually.
    
    # 3. Return normalized path for current OS
...
if LOCAL_SHARE_ROOT:
    ALLOWED_ROOTS.insert(0, str(LOCAL_SHARE_ROOT))
...
def log_config_change(user: str, action: str, details: str):
    ...
    log_dir = resolve_path(cfg.status_log_dir)
...
def log_status_event(cfg: AppConfig, category: str, box: BoxConfig, prev: str, curr: str, reason: str, context: str = "") -> None:
    ...
    out_dir = resolve_path((cfg.status_log_dir or "").strip())
    if not out_dir:
        return
    try:
...
def read_csv_rows(path: str) -> List[dict]:
    ...
    if not rpath or not os.path.exists(rpath):
        return rows
    with open(rpath, "r", newline="", encoding="utf-8-sig") as f:
...
def read_log(path: str) -> List[dict]:
    ...
    if not rpath or not os.path.exists(rpath):
        return out
    try:
        with open(rpath, "r", newline="", encoding="utf-8") as f:
...
class StatusEngine:
    ...
    def _sync_maintenance_dirs(self) -> None:
        ...
        for b in self.cfg.boxes:
            if b.csv_path:
                # resolve path to ensure we get a valid dirname on Linux
                rpath = resolve_path(b.csv_path)
                box_dirs[b.uid] = os.path.dirname(rpath)
    ...
    def _load_csv_rows(self, paths: Iterable[str]) -> Tuple[Dict[str, List[dict]], Dict[str, str]]:
        ...
        for path in paths:
            if not path:
                continue
            # Resolve path for file access, but keep cache key as original path
            rpath = resolve_path(path)
            try:
                stat = os.stat(rpath)
                mtime = stat.st_mtime
            ...
            if cache_entry and cache_entry.get("mtime") == mtime and cache_entry.get("size") == size:
                ...
                continue
            try:
                # read_csv_rows now handles resolution internally, but we can pass resolved or original.
                # Since we updated read_csv_rows to call resolve_path, passing original 'path' is fine!
                # WAIT, I updated read_csv_rows to call resolve_path(path).
                # So if I pass 'path' (UNC), it resolves it.
                # If I pass 'rpath' (resolved), it resolves it again (no-op).
                
                # However, os.stat needs rpath.
                
                parsed = read_csv_rows(path) # This calls resolve_path internally
                rows[path] = parsed
    ...
    def _maybe_run_daily_report(self, rows_by_path: Dict[str, List[dict]]) -> None:
        ...
        if getattr(self, "_first_refresh", False) or (now.hour, now.minute) >= (hh, mm):
            ...
            r_report_dir = resolve_path(cfg.report_dir)
            os.makedirs(r_report_dir, exist_ok=True)
            out_path = os.path.join(r_report_dir, f"LabManagerReport_{today_str}.csv")
            try:
                ...
                self._export_daily_logs(today_str, r_report_dir)
    ...
    def _export_daily_logs(self, today_str: str, report_dir: str):
        ...
        log_dir = resolve_path(self.cfg.status_log_dir)
        ...
        cf_dir = resolve_path((self.cfg.correction_factor_dir or "").strip())
        if cf_dir:
...
def _get_correction_factor_paths(cfg: AppConfig) -> Tuple[str, str]:
    ...
    cf_dir = resolve_path((cfg.correction_factor_dir or "").strip())
    if not cf_dir:
        return "", ""
