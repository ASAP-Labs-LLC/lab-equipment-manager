...
from PyQt5.QtWidgets import (
    ...
    QTabWidget, QDateEdit, QSpinBox, QTextEdit, QListWidgetItem, QAbstractItemView
)
...
class AddMaintenanceDialog(QDialog):
    def __init__(self, parent, box_title: str, default_kind: Optional[str] = None) -> None:
        ...
        self.kind_combo = QComboBox()
        if default_kind and default_kind.lower().startswith("cal"):
            self.name_edit.setText("Calibration"); self.name_edit.setReadOnly(True)
        if default_kind:
            idx = self.kind_combo.findText(default_kind, Qt.MatchFixedString)
            if idx >= 0:
    ...
    def get_data(self) -> Optional[Dict[str, object]]:
        ...
        return {
            "name": name,
            "kind": "calibration" if "cal" in kind else "pm",
            "start": start_dt,
...
class MachineMaintenanceTab(QWidget):
    def __init__(self, manager: MaintenanceManager, owner, box: BoxConfig) -> None:
        ...
        comment_box = QGroupBox("Comments")
        c_layout = QVBoxLayout(comment_box)
        self.comment_list = QListWidget()
        c_layout.addWidget(self.comment_list)
        form = QFormLayout()
        ...
        c_layout.addWidget(add_comment_btn)
    ...
    def _add_task(self, default_kind: str) -> None:
        ...
        if not data:
            return
        self.owner.add_maintenance_task(self.box, data['name'], data['kind'],
    ...
    def _complete_task(self) -> None:
        ...
        self.owner.complete_maintenance_task(task_id, user, comment)
        self.refresh()
