...
class MainWindow(QMainWindow):
    ...
    def open_settings(self) -> None:
        ...
        if dlg.exec_() == QDialog.Accepted:
            ...
            # Process Changeover QC reassignments
            reassignments = dlg.get_reassignments()
            if reassignments:
                count = 0
                for old_name, new_name in reassignments:
                    for box in self.cfg.boxes:
                        changed = False
                        ...
                        if changed:
                            count += 1
                if count > 0:
                    self.statusBar().showMessage(f"Changeover: Updated {count} machines.", 8000)
