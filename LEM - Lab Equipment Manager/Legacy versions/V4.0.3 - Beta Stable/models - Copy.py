...
class UserSpec:
    ...
    @staticmethod
    def from_dict(d: dict) -> "UserSpec":
        return UserSpec(username=str(d.get("username", "")), password=str(d.get("password", "")))
...
class ChecklistItem:
    ...
    text: str
    days_active: List[int] = field(default_factory=lambda: [0, 1, 2, 3, 4])  # 0=Mon, 4=Fri
    item_type: str = "item"  # "item", "header", "subtask"
    uid: str = field(default_factory=lambda: str(uuid.uuid4()))
    parent_uid: Optional[str] = None  # UID of parent task, None for top-level items
    ...
    @staticmethod
    def from_dict(d: dict) -> "ChecklistItem":
        return ChecklistItem(
            text=str(d.get("text", "")),
            days_active=list(d.get("days_active", [0, 1, 2, 3, 4])),
            item_type=str(d.get("item_type", "item")),
            uid=str(d.get("uid") or str(uuid.uuid4())),
            parent_uid=d.get("parent_uid"),  # Can be None
        )


@dataclass
class ChecklistSpec:
    """Configuration for a checklist."""
    uid: str
    name: str
    due_time: str  # "HH:MM" 24h
    ...
    def from_dict(d: dict) -> "ChecklistSpec":
        return ChecklistSpec(
            uid=str(d.get("uid", "")),
            name=str(d.get("name", "")),
            due_time=str(d.get("due_time", "17:00")),
            items=[ChecklistItem.from_dict(x) for x in d.get("items", [])],
        )
...
class AppConfig:
    ...
    checklists: List[ChecklistSpec] = field(default_factory=list)
    ...
    correction_factor_dir: str = ""  # directory for correction factor storage
    ...
    def serialize(self) -> dict:
        return {
            ...
            "checklists": [c.serialize() for c in self.checklists],
            ...
            "correction_factor_dir": str(self.correction_factor_dir),
    ...
    def from_dict(d: dict) -> "AppConfig":
        ...
        checklists_data = raw.get("checklists", [])
        ...
        return AppConfig(
            ...
            samples=samples,
            boxes=boxes,
            users=[UserSpec.from_dict(x) for x in users_data],
            checklists=[ChecklistSpec.from_dict(x) for x in checklists_data],
            ...
            correction_factor_dir=str(raw.get("correction_factor_dir", "")),
