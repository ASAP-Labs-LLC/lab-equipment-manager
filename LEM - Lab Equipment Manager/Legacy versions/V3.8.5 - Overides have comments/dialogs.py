...
from PyQt5.QtWidgets import (
    ...
    QTabWidget, QDateEdit, QSpinBox, QTextEdit, QListWidgetItem, QAbstractItemView, QSlider
)
import os
import shutil
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, parent, samples: List[SampleSpec],
        ...
        if not samples_editable:
            try:
                self.sample_id_column_edit.setEnabled(False)
                ...
                self.sample_id_val_edit.setEnabled(False)
                self.tests_table.setEnabled(False)
                self.add_test_btn.setEnabled(False)
                self.remove_test_btn.setEnabled(False)
            except Exception:
                pass
