...
class MainWindow(QMainWindow):
    ...
    def _apply_font_from_cfg(self) -> None:
        # Load imported fonts and apply chosen app font
        from PyQt5.QtWidgets import QApplication
        try:
            for p in (self.cfg.imported_fonts or []):
                try:
                    QFontDatabase.addApplicationFont(p)
                except Exception:
                    continue
            fam = (self.cfg.app_font_family or '').strip()
            size = int(self.cfg.app_font_size or 10)
            if fam:
    ...
    def open_settings(self) -> None:
        dlg = SettingsDialog(
            ...
            app_font_family=self.cfg.app_font_family,
            app_font_size=self.cfg.app_font_size,
        )
        if dlg.exec_() == QDialog.Accepted:
            ...
            if new_mode == 'custom':
                ...
                try:
                    ...
                    pass
            fam, fsize = dlg.get_font_settings()
            new_fonts = dlg.get_imported_fonts()
            if new_fonts:
                # Merge unique
                existing = set(self.cfg.imported_fonts or [])
                for p in new_fonts:
                    if p not in existing:
                        existing.add(p)
                self.cfg.imported_fonts = list(existing)
            self.cfg.app_font_family = fam
            self.cfg.app_font_size = int(fsize)
