...
class MachineBoxItem(QGraphicsRectItem):
    def __init__(self, box: BoxConfig):
        ...
        self._last_override = self.box.manual_override
        self.warningItem: Optional[QGraphicsTextItem] = None
    ...
    def _refresh_text_layout(self) -> None:
        ...
        # quick-fit: try one smaller size, then truncate instead of many passes
        if self.subItem.boundingRect().height() > avail_h:
            f.setPointSize(base_pt - 1)
            self.subItem.setFont(f)
        if self.subItem.boundingRect().height() > avail_h:
            trimmed = self._truncate_lines_to_height(raw_lines, avail_h)
            self.subItem.setPlainText("\n".join(trimmed))

    def _truncate_lines_to_height(self, lines: List[str], max_height: float) -> List[str]:
        if not lines:
            return []
        truncated = list(lines)
        while truncated:
            self.subItem.setPlainText("\n".join(truncated))
            if self.subItem.boundingRect().height() <= max_height:
                return truncated
    ...
    def set_status(self, status: str, reason: str, info_lines: List[str], warnings: Optional[List[str]] = None) -> None:
        new_warnings = list(warnings or [])
        if (
            status == self._status
            and reason == self._reason
            and info_lines == self._info_lines
            and new_warnings == self._warnings
            and self._last_override == self.box.manual_override
        ):
            return
        self._status = status
        self._reason = reason
    ...
    def _update_warning_icon(self) -> None:
        if self.warningItem is None:
            self.warningItem = QGraphicsTextItem("⚠", self)
            wf = QFont()
            wf.setPointSize(12)
            wf.setBold(True)
            self.warningItem.setFont(wf)
            self.warningItem.setDefaultTextColor(C("warning_icon", "#d35400"))
        has_warning = bool(self._warnings)
        self.warningItem.setVisible(has_warning)
        if has_warning:
            self.warningItem.setPos(
                self.rect().width() - self.warningItem.boundingRect().width() - 4,
                -4,
            )
        tooltip_parts: List[str] = []
        if self._reason:
            tooltip_parts.append(self._reason)
        if self._warnings:
            tooltip_parts.extend(self._warnings)
        self.setToolTip("\n".join(p for p in tooltip_parts if p) or "")
