...
from models import TestSpec, BoxConfig, STATUS_GREEN, STATUS_RED, STATUS_YELLOW, STATUS_UNKNOWN
...
class ParameterResult:
    def __init__(self, test: Optional[TestSpec],
...
def evaluate_box(box: BoxConfig,
    ...
    for tname in box.watched_tests:
        test = tests_by_name.get(tname)
        if test is None:
            param_results.append(ParameterResult(test=None, latest_value=None, in_spec=None,
                                                 low=None, high=None, note=f"Test '{tname}' not found."))
            any_fail = True
            continue

        matches: List[Tuple[dict, datetime, bool]] = []
        for r in rows:
            sid = ci_lookup(r, test.sample_id_col)
            if sid is not None and str(sid).strip() == test.sample_id_val:
                ts, used_parsed = best_row_time(r, box.timestamp_col, box.csv_path)
                matches.append((r, ts, used_parsed))

        if not matches:
            param_results.append(ParameterResult(test=test, latest_value=None, in_spec=None,
                                                 low=test.expected - test.k * test.std_dev,
                                                 high=test.expected + test.k * test.std_dev,
                                                 note="No matching rows for QC sample."))
        ...
        param_results.append(ParameterResult(test=test, latest_value=val, in_spec=in_spec,
                                             low=low, high=high, latest_time=latest_time))

    if any_fail:
        status = STATUS_RED
        reason = "At least one watched test is out of spec."
        last_good_qc = None
