...
# -*- coding: utf-8 -*-
"""
main_window.py — Main window, scene (grid), list view, toolbar, zoom, reporting, and orchestration.
"""
...
from models import AppConfig, BoxConfig, TestSpec, STATUS_DEAD, STATUS_SERVICE, STATUS_GREEN, STATUS_RED, STATUS_YELLOW
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.tests_by_name: Dict[str, TestSpec] = {t.name: t for t in self.cfg.tests}
    ...
    def add_box(self) -> None:
        dlg = BoxEditor(self, list(self.tests_by_name.values()), None)
    ...
    def edit_box(self, box: BoxConfig) -> None:
        dlg = BoxEditor(self, list(self.tests_by_name.values()), box)
        if dlg.exec_() == QDialog.Accepted:
    ...
    def open_box_info(self, box: BoxConfig, status: str, reason: str, lines: List[str]) -> None:
        ...
        eval_res = evaluate_box(box, self.tests_by_name, rows)
        ...
        InfoDialog(self, box, eval_res).exec_()
    ...
    def open_settings(self) -> None:
        dlg = SettingsDialog(
            self,
            list(self.tests_by_name.values()),
            report_enabled=self.cfg.report_enabled,
            report_time=self.cfg.report_time,
            report_dir=self.cfg.report_dir,
        )
        if dlg.exec_() == QDialog.Accepted:
            self.cfg.tests = dlg.get_tests()
            self.tests_by_name = {t.name: t for t in self.cfg.tests}
    ...
    def _on_rows_loaded(self, rows_by_path: Dict[str, List[dict]]) -> None:
        ...
        for box in self.cfg.boxes:
            ...
            eval_res: BoxEvaluation = evaluate_box(box, self.tests_by_name, rows)
            ...
            if eval_res.results:
                for pr in eval_res.results[:4]:
                    if pr.test:
                        ...
                        lines.append(f"{pr.test.name}: {vtxt}{units} {flag}  tol±{tol:.6g}  {rng}")
                    else:
    ...
    def _refresh_table(self) -> None:
        ...
        for box in self.cfg.boxes:
            ...
            self.table.setItem(row, 3, QTableWidgetItem(eval_res.last_good_qc.isoformat(sep=' ') if eval_res.last_good_qc else "—"))
            ...
            self.table.setItem(row, 6, QTableWidgetItem(", ".join(box.watched_tests) if box.watched_tests else "—"))
    ...
    def _build_report(self, rows_by_path: Dict[str, List[dict]]) -> Tuple[List[str], List[List[str]]]:
        headers = [
            ...
            "Test Name", "Expected", "k*σ", "Low", "High", "Latest Value", "In Spec", "Units"
        ]
        out_rows: List[List[str]] = []
        for box in self.cfg.boxes:
            rows = rows_by_path.get(box.csv_path, [])
            ev = evaluate_box(box, self.tests_by_name, rows)
            status = ev.status
            if box.manual_override == STATUS_DEAD:
                status = STATUS_DEAD
            elif box.manual_override == STATUS_SERVICE:
                ...
                for pr in ev.results:
                    if pr.test:
                        ...
                        out_rows.append([
                            ...
                            pr.test.name, f"{pr.test.expected:.6g}", f"{tol:.6g}",
