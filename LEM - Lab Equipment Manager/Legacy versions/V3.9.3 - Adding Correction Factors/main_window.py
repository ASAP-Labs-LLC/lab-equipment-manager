...
from models import AppConfig, BoxConfig, SampleSpec, STATUS_DEAD, STATUS_SERVICE, STATUS_GREEN, STATUS_YELLOW, STATUS_RED
from config_store import load_config, save_config
from data_source import CsvReadWorker, BoxEvaluation, build_sample_index, evaluate_box
...
def evaluate_with_cache(cache: Optional[LastSeenCache],
                        ...
                        row_time_cache: Optional[Dict[int, Tuple[datetime, str]]] = None) -> BoxEvaluation:
    """
    Evaluate a box using live CSV data, falling back to cached samples if the
    CSV lacks recent rows (e.g., rotation/reset).
    """
...
def remap_boxes_for_samples(boxes: List[BoxConfig], replacements: List[Tuple[str, str]]) -> int:
    """
    Update all watched_targets in-place to point at replacement sample names.
    Returns number of pairs remapped.
    """
    mapping = {old: new for old, new in replacements if old and new and old != new}
    if not mapping:
        return 0
    changed = 0
    for box in boxes:
        for wt in box.watched_targets:
            if wt.sample in mapping:
                wt.sample = mapping[wt.sample]
                changed += 1
    return changed
...
class MainWindow(QMainWindow):
    ...
    def __init__(self) -> None:
        ...
        try:
            self._last_seen_cache = LastSeenCache(os.path.dirname(__file__))
            self._last_seen_cache.sync_config(self.cfg)
        except Exception:
            self._last_seen_cache = None
    ...
    def save_config(self) -> None:
        ...
        try:
            ...
            pass
        if self._last_seen_cache:
            try:
                self._last_seen_cache.sync_config(self.cfg)
                self._last_seen_cache.flush()
            except Exception:
                pass
    ...
    def _prompt_manual_override(self, action_base: str, turn_on: bool) -> Optional[Tuple[str, str]]:
        if self.view_only:
            self._view_only_blocked("Manual overrides")
            return None
        dlg = QDialog(self)
        dlg.setModal(True)
    ...
    def _add_box_item(self, box: BoxConfig) -> None:
        item = MachineBoxItem(box, view_only=self.view_only)
    ...
    def _secure_add_box(self) -> None:
        if self._view_only_blocked("Adding boxes"):
            return
    ...
    def edit_box(self, box: BoxConfig) -> None:
        if self._view_only_blocked("Editing machines"):
            return
        # Admin password gate
        pwd, ok = QInputDialog.getText(self, "Password", "Enter admin password:", QLineEdit.Password)
        if not ok:
    ...
    def remove_box(self, uid: str) -> None:
        ...
        self._last_payload_by_uid.pop(uid, None)
        self.cfg.boxes = [b for b in self.cfg.boxes if b.uid != uid]
        if self._last_seen_cache:
            self._last_seen_cache.remove_box(uid)
            self._last_seen_cache.flush()
        self._sync_maintenance_dirs()
        self.save_config()
        self.refresh_all()
    ...
    def open_box_info(self, box: BoxConfig, status: str, reason: str, lines: List[str]) -> None:
        ...
        ev = evaluate_with_cache(self._last_seen_cache, box, self.samples_by_name, self.sample_id_column, rows)
    ...
    def open_settings(self) -> None:
        if self._view_only_blocked("Settings"):
            return
        dlg = SettingsDialog(
            self,
            ...
            correction_factor_dir=self.cfg.correction_factor_dir,
        ...
        try:
            ...
            pass
        if dlg.exec_() == QDialog.Accepted:
            replacements = dlg.get_replacements()
            ...
            self.cfg.correction_factor_dir = dlg.get_correction_factor_dir()
            try:
            ...
            remapped = remap_boxes_for_samples(self.cfg.boxes, replacements)
            if remapped:
                try:
                    self.statusBar().showMessage(f"Updated {remapped} watched targets to replacement samples.", 6000)
                except Exception:
                    pass
            self._apply_font_from_cfg()
            self.save_config()
    ...
    def _on_rows_loaded(self, rows_by_path: Dict[str, List[dict]]) -> None:
        ...
        for box in self.cfg.boxes:
            ...
            ev: BoxEvaluation = evaluate_with_cache(self._last_seen_cache, box, self.samples_by_name, self.sample_id_column, rows)
            ...
            if status == STATUS_GREEN and ev.last_good_qc and not ev.from_cache:
                if (datetime.now() - ev.last_good_qc) > timedelta(hours=box.qc_expire_hours):
            ...
            if ev.results:
                for pr in ev.results[:4]:
                    if pr.test:
                        ...
                        flag = '' if pr.in_spec is None else ('OK' if pr.in_spec else 'FAIL')
                        flag_part = f" {flag}" if flag else ''
                        units = f" {pr.test.units}" if pr.test.units else ''
                        cache_part = ' (cached)' if pr.from_cache or ev.from_cache else ''
                        source_tag = getattr(pr, 'timestamp_source', '')
                        source_part = f" [{source_tag}]" if source_tag else ''
                        lines.append(f"{pr.test.name}: {vtxt}{units}{cache_part}{flag_part} +/-{tol:.6g} {rng}{source_part}")
                    else:
            ...
            if ev.last_good_qc:
                age = now - ev.last_good_qc
                cache_note = ' (cached)' if ev.from_cache else ''
                lines.append(f"In spec: {human_tdelta(age)} ago{cache_note}")
            elif ev.latest_match_time:
                age = now - ev.latest_match_time
                cache_note = ' (cached)' if ev.from_cache else ''
                lines.append(f"Last row: {human_tdelta(age)} ago{cache_note}")

            if ev.from_cache and ev.latest_match_time:
                lines.append(f"Last detected sample: {ev.latest_match_time.strftime('%Y-%m-%d %H:%M')} (cached)")
            item = self.box_items.get(box.uid)
            if item:
...
class MainWindow(QMainWindow):
    def __init__(self, view_only: bool = False) -> None:
        super().__init__()
        self.view_only = bool(view_only)
        title = "Lab Manager Viewer" if self.view_only else "Lab Manager Map"
        ...
        self.box_items: Dict[str, MachineBoxItem] = {}
        self._last_rows_cache: Dict[str, List[dict]] = {}
        self._sample_index_cache: Dict[str, Dict[str, List[dict]]] = {}
        self._row_time_cache: Dict[str, Dict[int, Tuple[datetime, str]]] = {}
        self._csv_mtimes: Dict[str, float] = {}
        self._prefetched_rows: Dict[str, List[dict]] = {}
        self._prefetched_mtimes: Dict[str, float] = {}
        self._last_payload_by_uid: Dict[str, Tuple[str, str, Tuple[str, ...]]] = {}
    ...
    def _view_only_blocked(self, action: str) -> bool:
    ...
    def _setup_toolbar(self) -> None:
        ...
        if not self.view_only:
            ...
            tb.addAction(refresh_act)
            tb.addSeparator()
            mode_lbl = QLabel(" Viewer Mode "); mode_lbl.setStyleSheet("font-weight: bold;")
    ...
    def remove_box(self, uid: str) -> None:
        if self._view_only_blocked("Removing machines"):
            return
        # Admin password gate
        pwd, ok = QInputDialog.getText(self, "Password", "Enter admin password:", QLineEdit.Password)
        if not ok:
    ...
    def open_box_info(self, box: BoxConfig, status: str, reason: str, lines: List[str]) -> None:
        rows = self._last_rows_cache.get(box.csv_path, [])
        sample_index = self._sample_index_cache.get(box.csv_path)
        time_cache = self._row_time_cache.get(box.csv_path)
        eval_res = evaluate_with_cache(
    ...
    def refresh_all(self) -> None:
        ...
        prefetched_mtimes: Dict[str, float] = {}
        to_load: List[str] = []
        for path in paths:
            try:
            ...
            if (
                mtime is not None
                and path in self._last_rows_cache
                and self._csv_mtimes.get(path) == mtime
            ):
                prefetched[path] = self._last_rows_cache[path]
                prefetched_mtimes[path] = mtime
            else:
                to_load.append(path)

        self._prefetched_rows = prefetched
        self._prefetched_mtimes = prefetched_mtimes

        if not to_load:
            self._refresh_pending = False
            self._on_rows_loaded({})
            return

        self._refresh_pending = False
        self._start_csv_refresh(to_load)
    ...
    def _on_rows_loaded(self, rows_by_path: Dict[str, List[dict]]) -> None:
        combined_rows: Dict[str, List[dict]] = dict(self._prefetched_rows)
        combined_rows.update(rows_by_path)
        self._prefetched_rows = {}
        self._last_rows_cache = combined_rows

        for path, mtime in self._prefetched_mtimes.items():
            self._csv_mtimes[path] = mtime
        self._prefetched_mtimes = {}
        for path in rows_by_path:
            try:
                self._csv_mtimes[path] = os.path.getmtime(path)
            except Exception:
                pass

        self._sample_index_cache = {
            path: build_sample_index(rows, self.sample_id_column) for path, rows in combined_rows.items()
        }
        self._row_time_cache = {path: {} for path in combined_rows}

        # Startup catch-up: export today's report immediately if missed (before drawing today's statuses)
        if self._first_run:
            self._maybe_run_daily_report(combined_rows, force_if_missed=True)
            self._first_run = False

        self._ensure_first_inspec_epoch()

        for box in self.cfg.boxes:
            rows = combined_rows.get(box.csv_path, [])
            sample_index = self._sample_index_cache.get(box.csv_path)
            time_cache = self._row_time_cache.get(box.csv_path)
            eval_res: BoxEvaluation = evaluate_with_cache(
                self._last_seen_cache,
                box,
                self.samples_by_name,
                self.sample_id_column,
            ...
            if status == STATUS_GREEN and effective_last_good and not eval_res.from_cache:
            ...
            if eval_res.results:
                for pr in eval_res.results[:4]:
                    if pr.test:
                        ...
                        flag_part = f" {flag}" if flag else ''
                        units = f" {pr.test.units}" if pr.test.units else ''
                        cache_part = ' (cached)' if pr.from_cache or eval_res.from_cache else ''
                        source_tag = getattr(pr, 'timestamp_source', '')
                        source_part = f" [{source_tag}]" if source_tag else ''
                        lines.append(f"{pr.test.name}: {vtxt}{units}{cache_part}{flag_part} +/-{tol:.6g} {rng}{source_part}")
                    else:
                ...
                if len(eval_res.results) > 4:
                    lines.append(f"+{len(eval_res.results)-4} more...")
            else:
                lines.append('(no tests)')

            if effective_last_good:
                age = datetime.now() - effective_last_good
                cache_note = ' (cached)' if eval_res.from_cache else ''
                lines.append(f"In spec: {human_tdelta(age)} ago{cache_note}")
            elif eval_res.latest_match_time:
                age = datetime.now() - eval_res.latest_match_time
                cache_note = ' (cached)' if eval_res.from_cache else ''
                lines.append(f"Last row: {human_tdelta(age)} ago{cache_note}")

            if eval_res.from_cache and eval_res.latest_match_time:
                lines.append(f"Last detected sample: {eval_res.latest_match_time.strftime('%Y-%m-%d %H:%M')} (cached)")
            item = self.box_items.get(box.uid)
            payload = (status, reason, tuple(lines))
            prev_payload = self._last_payload_by_uid.get(box.uid)
            if item and payload != prev_payload:
                item.set_status(status, reason, lines)
            self._last_payload_by_uid[box.uid] = payload
        ...
        self._maybe_run_daily_report(combined_rows)
    ...
    def _refresh_table(self) -> None:
        ...
        for box in self.cfg.boxes:
            ...
            if eval_res.last_good_qc and eval_res.status in (STATUS_GREEN, STATUS_YELLOW):
                ttl = (eval_res.last_good_qc + timedelta(hours=box.qc_expire_hours)) - datetime.now()
            ...
            for pr in eval_res.results:
                if pr.test and pr.latest_value is not None:
                    ...
                    try:
                        ...
                        pass
                    break
            if eval_res.from_cache and latest_txt != '-':
                latest_txt = f"{latest_txt} (cached)"
    ...
    def _build_report(self, rows_by_path: Dict[str, List[dict]]) -> Tuple[List[str], List[List[str]]]:
        ...
        sample_indexes = {
            path: build_sample_index(rows, self.sample_id_column) for path, rows in rows_by_path.items()
        }
                ...
                for pr in ev.results:
                    ...
                    if pr.test:
                        ...
                        out_rows.append([
                            ...
                            low, high, latest, ts_source, insp, units
                        ])
                    else:
                        out_rows.append([
                            box.title, box.uid, status, (box.manual_override or ""),
...
class MaintenancePanel(QDockWidget):
    ...
    def __init__(self, manager: MaintenanceManager, owner: 'MainWindow') -> None:
        ...
        self.owner = owner
        self._view_only = bool(getattr(owner, "view_only", False))
        ...
        if not self._view_only:
            ...
            self.active_table.setContextMenuPolicy(Qt.NoContextMenu)
...
def _mw_add_maintenance_task(self, box: BoxConfig, name: str, kind: str,
    ...
    if getattr(self, "view_only", False):
        self._view_only_blocked("Adding maintenance tasks")
        return False
...
def _mw_start_maintenance_task(self, task_id: str) -> None:
    if getattr(self, "view_only", False):
        self._view_only_blocked("Starting maintenance tasks")
        return
...
def _mw_complete_maintenance_task(self, task_id: str, user: str, comment: str) -> None:
    if getattr(self, "view_only", False):
        self._view_only_blocked("Completing maintenance tasks")
        return
...
def _mw_add_maintenance_comment(self, box: BoxConfig, comment: str, user: str) -> None:
    if getattr(self, "view_only", False):
        self._view_only_blocked("Adding maintenance comments")
        return
...
def _mw_open_task_from_panel(self, box_uid: str, task_id: str) -> None:
    ...
    if not box:
        return
    rows = getattr(self, "_last_rows_cache", {}).get(box.csv_path, [])
    sample_index = getattr(self, "_sample_index_cache", {}).get(box.csv_path)
    time_cache = getattr(self, "_row_time_cache", {}).get(box.csv_path)
    eval_res = evaluate_with_cache(
        self._last_seen_cache,
        box,
        self.samples_by_name,
...
def _mw_delete_maintenance_task(self, task_id: str, user: str, reason: str) -> bool:
    if getattr(self, "view_only", False):
        self._view_only_blocked("Deleting maintenance tasks")
        return False
