...
from PyQt5.QtWidgets import (
    ...
    QTabWidget, QDateEdit, QSpinBox, QTextEdit, QListWidgetItem, QAbstractItemView, QSlider, QInputDialog, QMenu
)
import os
import shutil
...
class SettingsDialog(QDialog):
    ...
    def __init__(self, parent, samples: List[SampleSpec],
        ...
        self._replacement_pairs: List[Tuple[str, str]] = []
        ...
        self.sample_list.customContextMenuRequested.connect(self._on_sample_context_menu)
        ...
        self.scale_slider.setValue(int(max(50, min(200, ui_scale * 100))))
        self.scale_value = QLabel(f"{self.scale_slider.value()}%")
        ...
        def import_theme():
            ...
            try:
                ...
                if self.theme_cb.findText(label) == -1:
                    self.theme_cb.addItem(label)
    ...
    def _set_sample_manager_enabled(self, enabled: bool) -> None:
        try:
            for fn in getattr(self, '_sample_manager_controls', []):
                fn(bool(enabled))
        except Exception:
            pass

    def get_map_locked(self) -> bool:
        try:
            return bool(self.map_lock_cb.isChecked())
        except Exception:
            return False
    ...
    def _on_sample_context_menu(self, pos) -> None:
        row = self.sample_list.indexAt(pos).row()
        if row < 0 or row >= len(self._samples):
            return
        menu = QMenu(self)
        repl = menu.addAction("New replacement sample")
        chosen = menu.exec_(self.sample_list.viewport().mapToGlobal(pos))
        if chosen == repl:
            self._create_replacement_sample(row)

    def _unique_sample_name(self, base: str) -> str:
        candidate = base or "Sample"
        existing = {s.name for s in self._samples}
    ...
    def _prompt_replacement_inputs(self, old: SampleSpec) -> Optional[Tuple[str, str]]:
        while True:
            default_name = self._unique_sample_name(f"{old.name} (new)")
            new_name, ok = QInputDialog.getText(
                self, "Replacement Sample", "New sample name:", QLineEdit.Normal, default_name
            )
            if not ok:
                return None
            new_name = new_name.strip()
            if not new_name:
                QMessageBox.warning(self, "Name required", "Please enter a name for the replacement sample.")
                continue
            if any((s.name == new_name) for s in self._samples):
                QMessageBox.warning(self, "Duplicate name", "A sample with that name already exists.")
                continue
            break
        ...
        while True:
            new_id, ok = QInputDialog.getText(
                self,
                "Replacement Sample ID",
                "New sample ID value (for CSV matching):",
                QLineEdit.Normal,
                old.sample_id_val,
            )
            if not ok:
    ...
    def _create_replacement_sample(self, row: int) -> None:
        self._save_current_sample()
        if not (0 <= row < len(self._samples)):
            return
        src = self._samples[row]
        details = self._prompt_replacement_inputs(src)
        if not details:
            return
        new_name, new_id = details
        clone = SampleSpec.from_dict(src.serialize())
        clone.name = new_name
        clone.sample_id_val = new_id
        self._samples.append(clone)
        self._replacement_pairs.append((src.name, new_name))
    ...
    def get_replacements(self) -> List[Tuple[str, str]]:
        """Return (old_name, new_name) pairs requested during this session."""
        return list(self._replacement_pairs)
