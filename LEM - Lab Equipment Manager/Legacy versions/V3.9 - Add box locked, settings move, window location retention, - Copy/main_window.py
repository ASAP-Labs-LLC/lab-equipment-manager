...
def evaluate_with_cache(cache: Optional[LastSeenCache],
    ...
    """
    evaluation = evaluate_box(box, samples_by_name, sample_id_column, rows)
    if cache is None:
        return evaluation
...
class MainWindow(QMainWindow):
    ...
    def open_box_info(self, box: BoxConfig, status: str, reason: str, lines: List[str]) -> None:
        rows = self._last_rows_cache.get(box.csv_path, [])
        eval_res = evaluate_with_cache(self._last_seen_cache, box, self.samples_by_name, self.sample_id_column, rows)
        if box.manual_override == STATUS_DEAD:
    ...
    def _on_rows_loaded(self, rows_by_path: Dict[str, List[dict]]) -> None:
        ...
        for box in self.cfg.boxes:
            rows = rows_by_path.get(box.csv_path, [])
            eval_res: BoxEvaluation = evaluate_with_cache(self._last_seen_cache, box, self.samples_by_name, self.sample_id_column, rows)
