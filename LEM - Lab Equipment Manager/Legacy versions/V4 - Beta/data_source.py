...
def evaluate_box(box: BoxConfig,
                 ...
                 file_mtime: Optional[float] = None) -> BoxEvaluation:
    """
    ...
    for target in box.watched_targets:
        ...
        for r in candidate_rows:
            # Skip rows with blank/empty values for the target test column
            raw_val = ci_lookup(r, test.value_col)
            if raw_val is None or str(raw_val).strip() == "":
                continue

            if row_time_cache is not None:
                cached = row_time_cache.get(id(r))
            else:
                cached = None
            if cached:
                ts, ts_source = cached
