        ...
        try:
            ...
            if day_data:
                ...
                with open(cl_path, "w", newline="", encoding="utf-8") as f:
                    ...
                    # Sort by checklist title then item index
                    # items are keys (uid, idx)
                    sorted_keys = sorted(day_data.keys(), key=lambda k: (cl_map.get(k[0]).name if cl_map.get(k[0]) else "", k[1]))
                    
                    for uid, idx in sorted_keys:
                        entry = day_data[(uid, idx)]
                        cl = cl_map.get(uid)
                        title = cl.name if cl else uid
                        # Try to get item text from config
                        item_text = f"Item {idx+1}"
                        if cl and idx < len(cl.items):
                            item_text = cl.items[idx].text
...
# Structure: { date_str: { (checklist_uid, item_idx): { "user": str, "time": str, "checked": bool } } }
CHECKLIST_STATE: Dict[str, Dict[Tuple[str, int], dict]] = {}
        ...
        with open(CHECKLIST_STATE_FILE, "r", encoding="utf-8") as f:
            ...
            for date_key, items in raw.items():
                ...
                for key_str, val in items.items():
                    try:
                        uid, idx_str = key_str.rsplit("|", 1)
                        CHECKLIST_STATE[date_key][(uid, int(idx_str))] = val
...
def api_get_checklists():
    ...
    serial_state = {}
    for (uid, idx), val in state.items():
        serial_state[f"{uid}_{idx}"] = val
...
def api_toggle_checklist_item():
    ...
    data = json_body()
    uid = data.get("uid")
    idx = data.get("index") # integer index of item in the checklist
    checked = data.get("checked") # boolean
    
    if uid is None or idx is None or checked is None:
    ...
    # Also log to report?
    # log_config_change(user, "Checklist", f"{'Checked' if checked else 'Unchecked'} item {idx} in {uid}")
...
def api_save_checklists():
    ...
    def mutate(cfg: AppConfig):
        ...
        log_config_change(get_current_user(), "Config Update", "Updated Checklists")
